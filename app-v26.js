/* Cameracal AFMA Report Tool v26 LCD overlay position fixed */
/* Cameracal AFMA Report Tool v25 camera/menu sizing corrected */
/* Cameracal AFMA Report Tool v24 camera size/proportion fix */
/* Cameracal AFMA Report Tool v23 restored controls and clean dynamic camera */
...
.afhead{height:22%;background:#6f256f;padding:1.1mm 1.5mm;font-weight:700;font-size:8.5pt}
.aflens{height:19%;font-weight:700;padding:1mm 1.5mm;font-size:5.8pt;border-bottom:1px solid #333;white-space:nowrap;overflow:hidden}
.afrow{height:29%;display:grid;grid-template-columns:9mm 1fr 9mm;align-items:center;padding:0 1.5mm}
.afrow>b{text-align:center;font-size:8.5pt}
.scale{height:11mm;position:relative}
.nums{display:flex;justify-content:space-between;font-size:4.8pt;margin-bottom:.6mm}
...
.cert{height:23mm;margin-top:2mm;border:1px solid #0f4c81;border-radius:2mm;display:grid;grid-template-columns:25mm 1fr 56mm 54mm;gap:4mm;align-items:center;padding:2.5mm;font-size:7.7pt}
.seal{width:15mm;height:15mm;border-radius:50%;background:#0f4c81;color:white;font-size:18pt;font-weight:700;display:flex;align-items:center;justify-content:center;margin:auto}
.signature{line-height:1;font-family:"Brush Script MT","Segoe Script",cursive;font-size:14pt;border-bottom:1px solid #222;padding-bottom:.5mm}
.disclaimer{
  position:absolute;
  left:7mm;
  right:7mm;
...
/* v23 robust report generator and dynamic lens/camera rendering */
function cc23_escape(v){return String(v??'').replace(/[&<>"']/g,s=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[s]));}
function cc23_get(id){const n=document.getElementById(id);return n?n.value:'';}
function cc23_signed(v){v=Number(v||0);return v>0?`+${v}`:`${v}`;}
function cc23_cm(v){const c=Number(v||0)*0.75;return `${c>0?'+':''}${c.toFixed(2)} cm`;}
function cc23_focus(v){v=Number(v||0);return v>0?'Back focus':v<0?'Front focus':'Correct focus';}
function cc23_cls(v){v=Number(v||0);return v>0?'green':v<0?'red':'';}
function cc23_ap(v){return v&&String(v).trim()?String(v):'Not specified';}
function cc23_logo(){const img=document.querySelector('.logo');return img?img.src:'logo.png';}
function cc23_camera(){const img=document.querySelector('.camera img, .cameraPhoto');return img?img.src:'camera-base-clean.jpeg';}

function cc23_afMenu(single,w,t,lens){
  function row(label,value){
    let ticks='';
    for(let n=-20;n<=20;n++){
      const left=((n+20)/40)*100;
    ...
    const ptr=((Number(value)+20)/40)*100;
    return `<div class="afrow"><b>${label}</b><div class="scale"><div class="nums"><span>-20</span><span>-10</span><span>0</span><span>+10</span><span>+20</span></div><div class="rail">${ticks}<i style="left:${ptr}%"></i></div></div><b>${cc23_signed(value)}</b></div>`;
  }
  return `<div class="afmenu"><div class="afhead">AF Microadjustment</div><div class="aflens">[00] ${cc23_escape(lens).slice(0,34)}</div>${single?row('AF',w):row('W',w)+row('T',t)}</div>`;
}

function cc23_printReport(){
  const single=cc23_get('mode')==='single';
  const ref=cc23_get('jobRef')||'CC-Report';
  const date=cc23_get('reportDate')||new Date().toISOString().slice(0,10);
  const technician=cc23_get('technicianName')||'Cameracal Services';
  const customer=cc23_get('customerName')||'Not supplied';
  const cameraDesc=cc23_get('cameraDescription')||'Not supplied';
  const lens=cc23_get('lensName')||'Not supplied';
  const distance=(cc23_get('distance')||'10')+'m';
  const reportId=`${ref}-${date.replaceAll('-','')}`;
  const logo=cc23_logo();
  const camera=cc23_camera();

  const wf=single?Number(cc23_get('singleFocal')||70):Number(cc23_get('wideFocal')||24);
  const tf=single?null:Number(cc23_get('teleFocal')||70);
  const bw=single?Number(cc23_get('beforeSingle')||0):Number(cc23_get('beforeWide')||0);
  const bt=single?null:Number(cc23_get('beforeTele')||0);
  const aw=single?Number(cc23_get('singleValue')||0):Number(cc23_get('wideValue')||0);
  const at=single?null:Number(cc23_get('teleValue')||0);
  const wideOpt=single?cc23_ap(cc23_get('singleOptAperture')):cc23_ap(cc23_get('wideOptAperture'));
  const teleOpt=single?'':cc23_ap(cc23_get('teleOptAperture'));
  const notes=cc23_get('techNotes')||'No additional notes entered.';

  const teleRows=single?'':`<tr><td><b>Tele (${tf}mm)</b></td><td class="${cc23_cls(bt)}">${cc23_signed(bt)} / ${cc23_cm(bt)}<br><small>${cc23_focus(bt)}</small></td><td class="${cc23_cls(at)}">${cc23_signed(at)} / ${cc23_cm(at)}<br><small>${cc23_focus(at)}</small></td></tr>`;
  const optTele=single?'':`<tr><td><b>Tele (${tf}mm)</b></td><td>${cc23_escape(teleOpt)}</td></tr>`;
  const finalValues=single?`<b>Single (${wf}mm):</b><br><span class="${cc23_cls(aw)}">${cc23_signed(aw)} / ${cc23_cm(aw)}</span><br><small>${cc23_focus(aw)}</small>`:`<b>Wide (${wf}mm):</b> <span class="${cc23_cls(aw)}">${cc23_signed(aw)} / ${cc23_cm(aw)}</span><br><small>${cc23_focus(aw)}</small><hr><b>Tele (${tf}mm):</b> <span class="${cc23_cls(at)}">${cc23_signed(at)} / ${cc23_cm(at)}</span><br><small>${cc23_focus(at)}</small>`;

  const report=`<!doctype html><html><head><meta charset="utf-8"><title>${cc23_escape(ref)} Calibration Report</title><style>
@page{size:A4 landscape;margin:5mm}*{box-sizing:border-box}html,body{margin:0;padding:0;background:white;font-family:Arial,Helvetica,sans-serif;color:#101827;-webkit-print-color-adjust:exact;print-color-adjust:exact}
.sheet{width:287mm;height:200mm;border:1.7px solid #0f4c81;border-radius:4mm;padding:5mm 7mm 0;overflow:hidden;position:relative}
.header{height:21mm;display:grid;grid-template-columns:74mm 1fr;align-items:center;border-bottom:1.2px solid #0f4c81;padding-bottom:2mm}.logo{width:60mm;height:15mm;object-fit:contain;object-position:left center}.title{text-align:right;color:#0f3970}.title h1{margin:0;font-size:17pt;line-height:1}.title h2{margin:2mm 0 0;font-size:10pt}
.info{height:34mm;margin-top:3mm;border:1px solid #0f4c81;border-radius:2mm;padding:2.5mm 4mm;display:grid;grid-template-columns:1fr 1fr;gap:8mm;font-size:7.8pt}.infoRow{display:grid;grid-template-columns:35mm 1fr;margin-bottom:1.6mm}.infoRow b{color:#0b3770}
.content{height:107mm;margin-top:3mm;display:grid;grid-template-columns:121mm 1fr;gap:5mm}.block{border:1px solid #c8d6e2;border-radius:1.4mm;overflow:hidden;margin-bottom:2.2mm;background:white}.block h3{margin:0;background:#0f4c81;color:white;padding:2mm 3mm;font-size:8.8pt;text-transform:uppercase}
table{width:100%;border-collapse:collapse;font-size:7.7pt}th,td{border:1px solid #d7e0e8;padding:2mm;text-align:center;vertical-align:middle}th{background:#eef5fb;color:#0b3770;font-size:7.1pt}td:first-child,th:first-child{text-align:left}small{font-size:7pt;color:#111}.red{color:#c62828;font-weight:700}.green{color:#2e7d32;font-weight:700}
.twobox{display:grid;grid-template-columns:1fr 1fr;gap:3mm}.textbox{height:30mm;border:1px solid #c8d6e2;border-radius:1.4mm;overflow:hidden;font-size:7.6pt}.textbox h3{margin:0;background:#0f4c81;color:white;padding:2mm 3mm;font-size:8.5pt;text-transform:uppercase}.textbox p{margin:0;padding:2.5mm;line-height:1.3}
.cameraBlock{height:72mm}.cameraArea{position:relative;height:64mm;overflow:hidden;background:#111}.cameraArea img{width:100%;height:100%;object-fit:contain;object-position:center center;display:block;background:#111}
.afmenu{position:absolute;left:30.2%;top:23.9%;width:40%;height:48.5%;background:#050505;border:1px solid rgba(255,255,255,.4);color:white}.afhead{height:22%;background:#6f256f;padding:1.1mm 1.5mm;font-weight:700;font-size:8.5pt}.aflens{height:19%;font-weight:700;padding:1mm 1.5mm;font-size:5.8pt;border-bottom:1px solid #333;white-space:nowrap;overflow:hidden}.afrow{height:29%;display:grid;grid-template-columns:9mm 1fr 9mm;align-items:center;padding:0 1.5mm}.afrow>b{text-align:center;font-size:8.5pt}.scale{height:11mm;position:relative}.nums{display:flex;justify-content:space-between;font-size:4.8pt;margin-bottom:.6mm}.rail{position:absolute;left:0;right:0;top:5.5mm;border-top:1px solid white;height:6mm}.tick{position:absolute;top:-1.8mm;height:3.5mm;border-left:1px solid white}.tick.major{top:-2.6mm;height:5mm}.tick.zero{top:-3.5mm;height:7mm;border-left-width:2px}.rail i{position:absolute;top:-5.5mm;transform:translateX(-50%);width:0;height:0;border-left:3px solid transparent;border-right:3px solid transparent;border-top:6px solid white}
.caption{text-align:center;font-size:7pt;font-style:italic;margin-top:1mm}.final{height:29mm;border:1px solid #2e7d32;border-radius:2mm;overflow:hidden;background:#f4fbf2}.final h3{margin:0;background:#2e7d32;color:white;padding:2mm 3mm;font-size:9pt;text-transform:uppercase}.finalBody{display:grid;grid-template-columns:15mm 1fr 41mm;gap:3mm;padding:3mm;font-size:7.6pt;align-items:center}.check{width:12mm;height:12mm;border-radius:50%;background:#2e7d32;color:white;font-size:19pt;font-weight:700;display:flex;align-items:center;justify-content:center}.final h4{margin:0 0 1mm;color:#2e7d32;font-size:10.5pt}.finalVals{border:1px solid #95c788;border-radius:1.4mm;background:white;padding:2mm;font-size:7.2pt}
.cert{height:23mm;margin-top:2mm;border:1px solid #0f4c81;border-radius:2mm;display:grid;grid-template-columns:25mm 1fr 56mm 54mm;gap:4mm;align-items:center;padding:2.5mm;font-size:7.7pt}.seal{width:15mm;height:15mm;border-radius:50%;background:#0f4c81;color:white;font-size:18pt;font-weight:700;display:flex;align-items:center;justify-content:center;margin:auto}.signature{line-height:1;font-family:"Brush Script MT","Segoe Script",cursive;font-size:14pt;border-bottom:1px solid #222;padding-bottom:.5mm}
.disclaimer{position:absolute;left:7mm;right:7mm;bottom:8.7mm;height:8mm;color:#5f6b77;font-size:6.1pt;line-height:1.25;text-align:center;padding:0 4mm}.footer{position:absolute;left:0;right:0;bottom:0;height:8mm;background:#0f4c81;color:white;display:grid;grid-template-columns:1fr 1fr 1fr;align-items:center;padding:0 8mm;font-size:7.5pt}.footer div:nth-child(2){text-align:center}.footer div:nth-child(3){text-align:right}
</style></head><body><div class="sheet">
<div class="header"><img class="logo" src="${cc23_escape(logo)}"><div class="title"><h1>Camera & Lens Calibration Report</h1><h2>Cameracal Services</h2></div></div>
<div class="info"><div><div class="infoRow"><b>Customer:</b><span>${cc23_escape(customer)}</span></div><div class="infoRow"><b>Date:</b><span>${cc23_escape(date)}</span></div><div class="infoRow"><b>Camera:</b><span>${cc23_escape(cameraDesc)}</span></div><div class="infoRow"><b>Lens:</b><span>${cc23_escape(lens)}</span></div><div class="infoRow"><b>Subject Distance:</b><span>${cc23_escape(distance)}</span></div><div class="infoRow"><b>Adjustment Type:</b><span>${single?'Single':'Wide / Tele adjustment values'}</span></div></div><div><div class="infoRow"><b>Reference:</b><span>${cc23_escape(ref)}</span></div><div class="infoRow"><b>Technician:</b><span>${cc23_escape(technician)}</span></div><div class="infoRow"><b>Mode:</b><span>${single?'Single':'Wide / Tele'}</span></div><div class="infoRow"><b>Serial No:</b><span>—</span></div><div class="infoRow"><b>Standard:</b><span>Cameracal AFMA Procedure v1.0</span></div><div class="infoRow"><b>Report ID:</b><span>${cc23_escape(reportId)}</span></div></div></div>
<div class="content"><div><div class="block"><h3>Before / After Calibration Values</h3><table><tr><th>Position</th><th>Before AFMA / Offset</th><th>After AFMA / Offset</th></tr><tr><td><b>${single?'Single':'Wide'} (${wf}mm)</b></td><td class="${cc23_cls(bw)}">${cc23_signed(bw)} / ${cc23_cm(bw)}<br><small>${cc23_focus(bw)}</small></td><td class="${cc23_cls(aw)}">${cc23_signed(aw)} / ${cc23_cm(aw)}<br><small>${cc23_focus(aw)}</small></td></tr>${teleRows}</table></div><div class="block"><h3>Lens Optimisation Aperture (Recommended)</h3><table><tr><th>Position</th><th>Recommended aperture</th></tr><tr><td><b>${single?'Single':'Wide'} (${wf}mm)</b></td><td>${cc23_escape(wideOpt)}</td></tr>${optTele}</table></div><div class="twobox"><div class="textbox"><h3>Offset Calculation</h3><p>1 AF microadjustment = <b>0.75 cm</b>.<br>Positive values indicate back focus correction; negative values indicate front focus correction.</p></div><div class="textbox"><h3>Technician Notes</h3><p>${cc23_escape(notes)}</p></div></div></div><div><div class="block cameraBlock"><h3>Camera AF Microadjustment Settings</h3><div class="cameraArea"><img src="${cc23_escape(camera)}">${cc23_afMenu(single,aw,at,lens)}</div><div class="caption">Settings shown after calibration.</div></div><div class="final"><h3>Final Result</h3><div class="finalBody"><div class="check">✓</div><div><h4>Calibration Successful</h4><div>The autofocus system has been calibrated at the specified focal length${single?'':'s'}.</div><div>AF microadjustments are now optimised for improved focus accuracy.</div><b>The camera is ready for accurate and reliable performance.</b></div><div class="finalVals">${finalValues}</div></div></div></div></div>
<div class="cert"><div class="seal">✓</div><div><b>QUALITY CERTIFIED</b><br>This calibration has been performed and verified using Cameracal Services procedures.</div><div><b>Technician Signature:</b><br><div class="signature">${cc23_escape(technician)}</div>Date: ${cc23_escape(date)}</div><div><b>Reference:</b><br>${cc23_escape(ref)}<br><br><b>Report ID:</b><br>${cc23_escape(reportId)}</div></div>
...
const __cc23_btn=document.getElementById('pdfBtn'); if(__cc23_btn){ __cc23_btn.onclick=cc23_printReport; }
