/* Cameracal AFMA Report Tool v13 - PDF error handling and font fix */
/* Cameracal AFMA Report Tool v12 cache-busted JS */
...
async function pdf(){
  ...
  if(!window.jspdf || !window.jspdf.jsPDF){
    alert('PDF generator did not load. Please check your internet connection, refresh the page, and try again.');
    return;
  }
  ...
  function detailRows(rows,x,startY){
    let yy=startY;
    rows.forEach(r=>{
      doc.setFont('helvetica','bold'); doc.setTextColor(...blue); doc.text(r[0],x,yy);
      doc.setFont('helvetica','normal'); doc.setTextColor(...grey); doc.text(String(r[1]),x+34,yy,{maxWidth:55});
      yy+=6;
    });
  }
  ...
  doc.setFontSize(7.3); doc.setFont('helvetica','bold'); doc.setTextColor(...blue);
  ...
  function tableRow(label,b,a,yy){
    doc.setFontSize(7.5); doc.setFont('helvetica','bold'); doc.setTextColor(...grey); doc.text(label,tableX+3,yy);
    doc.setFont('helvetica','bold'); doc.setTextColor(b<0?red:green); doc.text(`${signed(b)} / ${cmT(b)}`,tableX+30,yy,{align:'left'});
    doc.setFont('helvetica','normal'); doc.setTextColor(...grey); doc.text(`(${label?focusShort(b):''})`,tableX+33,yy+4);
    doc.setFont('helvetica','bold'); doc.setTextColor(a<0?red:green); doc.text(`${signed(a)} / ${cmT(a)}`,tableX+59,yy,{align:'left'});
    doc.setFont('helvetica','normal'); doc.setTextColor(...grey); doc.text(`(${focusShort(a)})`,tableX+62,yy+4);
  }
  ...
  doc.setFontSize(7); doc.setFont('helvetica','italic'); doc.setTextColor(...grey);
  ...
  doc.setFontSize(7.2); doc.setFont('helvetica','bold'); doc.setTextColor(...blue);
  ...
  doc.setFont('helvetica','normal'); doc.setFontSize(7.8); doc.setTextColor(...grey);
  ...
  doc.setTextColor(...blue); doc.setFont('helvetica','bold'); doc.setFontSize(9);
  doc.text('QUALITY CERTIFIED',31,certY+9);
  doc.setTextColor(...grey); doc.setFont('helvetica','normal'); doc.setFontSize(7.8);
  ...
  doc.setTextColor(...blue); doc.setFont('helvetica','bold'); doc.text('Technician Signature:',84,certY+9);
  ...
  doc.setTextColor(...blue); doc.setFont('helvetica','bold'); doc.text('Reference:',142,certY+9);
  doc.setTextColor(...grey); doc.setFont('helvetica','normal'); doc.text($('jobRef').value||'',142,certY+15);
  doc.setTextColor(...blue); doc.setFont('helvetica','bold'); doc.text('Report ID:',142,certY+23);
  doc.setTextColor(...grey); doc.setFont('helvetica','normal'); doc.text(reportId,142,certY+29,{maxWidth:50});
  ...
  doc.save(($('jobRef').value||'Cameracal')+'-AFMA-Report.pdf');
    ...
    console.error(err);
    alert('The PDF could not be generated. Error: '+(err && err.message ? err.message : err));
  }
}
